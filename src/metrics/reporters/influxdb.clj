...
(comment "This reporter is coming in 2.8.0 of metrics clojure, but isn't yet released, so just copying it down for now :)")

(defn ^InfluxDbReporter reporter
  ([opts]
   (reporter default-registry opts))
  ([^MetricRegistry reg 
    {:keys [host hostname port prefix tags fields auth connect-timeout read-timeout
